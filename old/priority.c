...
void execute(int processIndex, int n)
{
  ...
  printf("\nExecuting Process %d with priority %d for %d seconds", p[processIndex].id, p[processIndex].priority, p[processIndex].burstTime);
  f++;
}

void prioritySchedule(pcb p[], int n)
{
  int currTime = 0;
  int index = 0;
  ...
  for (int i = 0; i < n; i++)
  {
    int maxPriority = INT_MIN; // assume minimum priority
    int maxPriorityProcessIndex;
    ...
    for (int index = 0; index < n; index++)
    {
      ...
      if (p[index].isDone == 0 && p[index].arrivalTime <= currTime && p[index].priority > maxPriority)
      {
        maxPriorityProcessIndex = index;
        maxPriority = p[maxPriorityProcessIndex].priority;
      }
    }
    // execute the process with minimum burst time and mark it as done
    execute(maxPriorityProcessIndex, n);
    // cpu will take the (burst time of the process) seconds from the current time to execute the process.
    currTime += p[maxPriorityProcessIndex].burstTime;
  }
}
